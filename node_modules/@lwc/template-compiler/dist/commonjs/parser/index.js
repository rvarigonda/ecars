...
function parseIf(ctx, _parse5Elm, parse5ElmLocation, parent, parsedAttr) {
...
function applyLwcSlotBindDirective(ctx, parsedAttr, element) {
    ...
    if (!ctx.config.enableScopedSlots) {
        ctx.throwOnNode(errors_1.ParserDiagnostics.INVALID_OPTS_LWC_SLOT_BIND, element);
    }
...
function parseScopedSlotFragment(ctx, parse5Elm, parse5ElmLocation, parent, parsedAttr) {
    ...
    if (!ctx.config.enableScopedSlots) {
        ctx.throwOnNode(errors_1.ParserDiagnostics.INVALID_OPTS_LWC_SLOT_DATA, slotDataAttr);
    }
    if (parse5Elm.tagName !== 'template') {
    ...
    if (slotAttr) {
        if (ast.isExpression(slotAttr.value)) {
            ctx.throwOnNode(errors_1.ParserDiagnostics.SLOT_ATTRIBUTE_CANNOT_BE_EXPRESSION, slotAttr);
        }
        else {
...
const ALLOWED_SLOT_ATTRIBUTES = new Set(['name', types_1.ElementDirectiveName.SlotBind]);
        ...
        if (!(0, shared_1.isNull)(slotFragment)) {
            slotFragment.children.forEach((child) => {
                ...
                if (ast.isElementDirective(child)) {
                    throw new Error(`Incorrect order of processing directives. ScopedSlotFragment` +
